...
def process_page(url, parent_url):
    '''
...
def get_pages(pages_section):
    ...
    for page in pages:
        pages_list.add(page.find('a').contents[0])
        pages_link_list.add((page.find('a'))['href'])
...
def get_next_page(pages_section, parent_url):
    ...
    for a_tag in a_tags:
        if a_tag.contents[0] == 'next page':
            ...
            return process_page(next_page_link, parent_url)
...
def process_all_pages(child_depth_links, max_category_limit, max_page_limit, parent_url, i):
    print('i:', i)
    file_limit = False
    child_cat_depth, child_cat_links_depth, child_page_depth, child_page_links_depth, child_done_links_depth = set(), set(), set(), set(), set()
    for url in list(child_depth_links):
        print('URl being scraped', url)
...
def process_no_depth_page(url, parent_url, max_page_limit, max_category_limit):
    file_limit = False
    child_cat, child_cat_links, child_page, child_page_links, child_done_links = process_page(url, parent_url,
                                                                                              max_page_limit,
                                                                                              max_category_limit)
